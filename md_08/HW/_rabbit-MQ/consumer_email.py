...
print(' [*] Waiting for for contact`s id. To exit press CTRL+C')
...
def callback(ch, method, properties, body) -> None:
    ...
    print(f" [x] Sending email to {contact.email}: {email_message}")
    time.sleep(2)
    # Update contact's is_delivered attribute to True
    contact.is_delivered = True
    contact.save()
    print(f" [x] Email was sent to {contact.email}")
